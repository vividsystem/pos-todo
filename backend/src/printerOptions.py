...
if __name__ == "__main__":
    TextOptions("right").set_default(p.driver)
    p.driver.textln("right aligned text")
    p._reset()

    for i in range(3):
        TextOptions(underlineType=i).set_default(p.driver)
    ...
    for i in ["a", "b"]:
        TextOptions(font=i).set_default(p.driver)
        p.driver.textln(f"type {i}")
    ...
    for i in range(9):
        TextOptions(density=i).set_default(p.driver)
